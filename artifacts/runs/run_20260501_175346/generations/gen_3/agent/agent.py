...
def _infer_language(code: str) -> str:
    lowered = code.lower()
    if any(tok in lowered for tok in ("->", "#include", "sizeof(", " char *", "int ", "void ")):
        return "c_cpp"
    if any(tok in lowered for tok in ("def ", "import ", "None", "self", "lambda")):
        return "python"
    return "unknown"


def _needs_forced_scan(code: str, finalize_args: dict, step: int, scan_used: bool) -> bool:
    if step != 0 or scan_used:
        return False
    label = _clean_label(finalize_args.get("label"))
    reasoning = (finalize_args.get("reasoning") or "").lower()
    code_lower = code.lower()
    code_len = len(code)

    null_speculation = any(
        phrase in reasoning
        for phrase in (
            "null-pointer",
            "null pointer",
    ...
    deref_heavy = code.count("->") >= 2 or code.count(".") >= 4
    wrapper_like = ("switch" in code_lower or "case " in code_lower or "return(" in code_lower or "return (" in code_lower)
    stateful_small_fn = code_len < 2200 and any(
        tok in code_lower
        for tok in (
            "sizeof(",
            "malloc",
    ...
    if label == "vulnerable" and null_speculation and deref_heavy:
        return True
    if label == "safe" and stateful_small_fn:
        return True
    if wrapper_like and (null_speculation or stateful_small_fn):
        return True
    return False
    ...
    for step in range(_MAX_STEPS):
        ...
        if not tool_calls:
            ...
            if coerced is not None:
                if _needs_forced_scan(code, coerced, step, scan_used):
                    scan_result = static_scan(code, language=_infer_language(code), run_external=False, timeout=5)
                    scan_used = True
                    history.append({
                        "role": "user",
                        "content": (
                            "Before finalizing, review this bounded static scan result and then call finalize. "
                            "Treat the scan as advisory, not decisive; prefer concrete local evidence over hypothetical caller/context assumptions.\n\n"
                            f"static_scan: {json.dumps(scan_result)}"
                        ),
                    })
                    continue
        ...
        if finalize_args is not None:
            if _needs_forced_scan(code, finalize_args, step, scan_used):
                scan_result = static_scan(code, language=_infer_language(code), run_external=False, timeout=5)
                scan_used = True
                history.append({
                    "role": "user",
                    "content": (
                        "Reconsider your tentative verdict using this bounded static scan before finalizing again. "
                        "Do not rely on hypothetical NULL/caller assumptions alone; also do not dismiss short stateful code without checking for concrete local size/count/state hazards.\n\n"
                        f"static_scan: {json.dumps(scan_result)}"
                    ),
                })
                continue
